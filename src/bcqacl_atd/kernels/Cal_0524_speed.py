#!/usr/bin/env python3
"""Cal_0524_speed SG-DVCL generator derived from Cal_0521_v2.

This file is an accuracy-preserving speed experiment.  The underlying
Cal_0423/Cal_0509 electrostatic and modal solves are unchanged; the speedups
come from avoiding unnecessary Touchstone write/read cycles when the caller
only needs an in-memory ``skrf.Network``, plus an exact in-process network
cache for repeated geometries.
...
_FAST_NETWORK_CACHE: OrderedDict[tuple[Any, ...], rf.Network] = OrderedDict()


def clear_fast_network_cache() -> None:
    """Clear the exact in-memory S4P network cache used by this speed variant."""
    _FAST_NETWORK_CACHE.clear()


def _freeze_for_key(value: Any) -> Any:
    """Convert nested values into a stable cache-key representation."""
...
def _fast_cache_get(key: tuple[Any, ...]) -> rf.Network | None:
...
def _fast_cache_put(key: tuple[Any, ...], ntw: rf.Network) -> rf.Network:
...
def _network_from_config_in_memory(params: Any, *, name: str | None = None) -> rf.Network:
    """Return the SG-DVCL S4P as an in-memory Network without Touchstone I/O."""
    import skrf as rf
...
def calculate_sgdvcl_s4p_from_geometry_fast(
    ...
    *,
    ...
    """Calculate the same S4P response but keep it in memory.

    This is the main Cal_0524 speed path.  It uses the same Cal_0509 config
    builder and the same Cal_0423 prepared-sweep bundle as Cal_0521_v2, then
    constructs the ``skrf.Network`` directly from the computed S matrices.  It
    therefore avoids the Touchstone write/read round trip while preserving the
...
def write_sgdvcl_s4p_from_geometry_fast(
    ...
    """Write the same S4P file without constructing an ``skrf.Network``.

    This path is intended for single-command cold starts where the caller only
    needs the Touchstone file.  It avoids importing scikit-rf and avoids reading
    the generated file back into memory.
    """
    fit_params = dict(CAL_0521_V2_HALF_TF_BASELINE_PARAMS)
    fit_params.update(params or {})
    fit_params = _validate_half_tf_params(fit_params)
...
def calculate_sgdvcl_s4p_from_half_tf_fast(
    ...
    *,
    ...
    """Fast in-memory variant of ``calculate_sgdvcl_s4p_from_half_tf``."""
    fit_params = dict(CAL_0521_V2_HALF_TF_BASELINE_PARAMS)
    fit_params.update(params or {})
    fit_params = _validate_half_tf_params(fit_params)
    geometry = half_tf_geometry_from_formula(
...
def write_sgdvcl_s4p_from_half_tf_fast(
    ...
    """Write a Cal_0524/0521-equivalent S4P without importing scikit-rf."""
    fit_params = dict(CAL_0521_V2_HALF_TF_BASELINE_PARAMS)
    fit_params.update(params or {})
    fit_params = _validate_half_tf_params(fit_params)
    geometry = half_tf_geometry_from_formula(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate one Cal_0524_speed SG-DVCL S4P.")
    ...
    parser.add_argument(
        "--memory-only",
        action="store_true",
        help="Use the in-memory speed path and do not write/read the Touchstone S4P file.",
    )
    parser.add_argument(
        "--write-only",
        action="store_true",
        help="Write the Touchstone S4P file but do not import/read it back as an skrf.Network.",
    )
...
def main() -> int:
    args = parse_args()
    if args.memory_only and args.write_only:
        raise ValueError("--memory-only and --write-only are mutually exclusive.")
    if args.write_only:
        params = {
            "frequency_hz": build_frequency_hz(
        ...
        out_path = write_sgdvcl_s4p_from_geometry_fast(
            WLine_um=args.width_um,
            L_MA_um=args.length_um,
            L_E1_um=args.length_um - 2.0,
            L_GND_open_um=args.length_um - 4.0,
            W_GND_open_um=args.gnd_width_factor * args.width_um,
            params=params,
        )
        print(out_path)
        print(
            "Generated Cal_0524_speed SG-DVCL S4P: "
    ...
    if args.memory_only:
        ...
        network = calculate_sgdvcl_s4p_from_geometry_fast(
            WLine_um=args.width_um,
            L_MA_um=args.length_um,
            L_E1_um=args.length_um - 2.0,
            L_GND_open_um=args.length_um - 4.0,
