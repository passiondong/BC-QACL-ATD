#!/usr/bin/env python3
"""0529_v2 transformer prediction pipeline: Cal_0529_v2 SG-DVCL S4P + L13-only S6P.

This script follows ``tf_analysis_pipeline_cli_0520.py`` but replaces the
SG-DVCL geometry generator with ``Cal_0529_v2``.  The L13 source, eight anchors,
...
def generate_cal_0529_v2_s4p(
    ...
    *,
    ...
    geometry = Cal_0529_v2.half_tf_geometry_from_formula(
        W_um,
        R,
        WlineR,
    ...
    filename = f"sgdvcl_cal0529v2_W{base.safe_tag(W_um)}_R{base.safe_tag(R)}_WlineR{base.safe_tag(WlineR)}.s4p"
    params = {
    ...
    ntw = Cal_0529_v2.calculate_sgdvcl_s4p_from_half_tf(W_um, R, WlineR, params=params)
...
def run_pipeline(
    ...
    *,
    ...
    if write_s4p_file or write_s6p_file:
        _s4p, s4p_path, geometry = generate_cal_0529_v2_s4p(
            W_um,
            R,
            WlineR,
    ...
    if write_s6p_file:
        ...
        stem = f"tf_pred_cal0529v2_l13only_W{base.safe_tag(W_um)}_R{base.safe_tag(R)}_WlineR{base.safe_tag(WlineR)}"
...
def _print_result(result: PipelineResult) -> None:
    ...
    print(
        f"Cal_0529_v2 geometry: L_base={result.geometry['L_base_um']:.9g} um, "
        f"input_WLine={result.geometry.get('input_WLine_um', result.geometry['Wline_um']):.9g} um, "
        f"actual_WLine={result.geometry.get('actual_WLine_um', result.geometry.get('Wline_SGDVCL_um', result.geometry['Wline_um'])):.9g} um, "
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate Cal_0529_v2 SG-DVCL S4P and L13-only predicted TF S6P.")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs") / "tf_analysis_pipeline_0529_v2")
    ...
    parser.add_argument("--no-s6p", action="store_true", help="Generate only the Cal_0529_v2 S4P.")
