#!/usr/bin/env python3
"""Cal_0521 SG-DVCL S4P generator for the L13-only six-port TF model.
...
The exported SG-DVCL geometry uses

    L_MA       = L_base
    L_E1       = L_base - 2 um
    L_GND_open = L_base - 4 um
    W_GND_open = 1.7 * Wline

and the modal line solver applies ``line_length_scale = 0.90`` internally.
...
# =============================================================================
# Cal_0521 half_TF-to-SG-DVCL parameters
# =============================================================================

# Updated by optimize_cal_0521_l13only_tf_s6p.py from the six-port golden-v4
# endpoint fit. Geometry and L13 remain fixed by the Cal_0521/tf_analysis
# 0521 policy; only numeric/mesh parameters below were optimized.
...
CAL_0521_HALF_TF_BASELINE_PARAMS: dict[str, Any] = {
...
OUTPUT_S4P_PATH = Path(
    ...
    r"\Cal_0521_outputs\Cal_0521_SGDVCL.s4p"
)
...
def _validate_half_tf_params(params: dict[str, Any]) -> dict[str, Any]:
    """Return Cal_0521 params after enforcing the fixed 0521 geometry policy."""
    checked = dict(params)
    ...
    for key, fixed_value in HALF_TF_FIXED_GEOMETRY_PARAMS.items():
        ...
        if abs(value - float(fixed_value)) > 1e-15:
            raise ValueError(f"{key} is fixed at {fixed_value:g} for Cal_0521.")
        checked[key] = float(fixed_value)
    return checked


def half_tf_geometry_from_formula(W_um: float, R: float, WlineR: float) -> dict[str, float]:
    """Derive the Cal_0521 SG-DVCL base geometry from transformer dimensions."""
    W_um = float(W_um)
    R = float(R)
    WlineR = float(WlineR)
    ...
    if L_base_um <= 4.0:
        raise ValueError(
            "Cal_0521 L_base must exceed 4 um so L_E1 and L_GND_open stay positive; "
    ...
    return {
        ...
        "W_GND_open_um": OPT_GND_WIDTH_FACTOR * Wline_um,
        "line_length_scale": OPT_LINE_LENGTH_SCALE,
...
def calculate_sgdvcl_from_length_width(
    *,
    ...
    fit_params = dict(CAL_0521_HALF_TF_BASELINE_PARAMS)
...
def calculate_sgdvcl_s4p_from_half_tf(
    ...
    """Calculate a Cal_0521 half-TF-derived straight SG-DVCL four-port."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate one Cal_0521 SG-DVCL S4P.")
...
def main() -> int:
    ...
    print(
        "Generated Cal_0521 SG-DVCL S4P: "
